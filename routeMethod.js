...
function drawNumberPlate(index){
	...
	for(let i = 0; i < number_plate.length; i++){
		if(pointEqual(number_plate[i], route[index].route_point[0]) && number_plate[i].index == index){
			exist = false;
		}
	}
	if(exist){
...
*/
function reFindRoute(now_X, now_Y, goto_X, goto_Y, index, lock) {
...
/*
  ...
  return index
*/
exports.findIndex = function(robot_ID, socket){
	for(let i = 0; i < point.length; i++){
...
/*
  ...
  會將尋找好的路徑存進route Array中
*/
exports.findRoute = function(now_X, now_Y, goto_X, goto_Y, robot_ID, index) {
	var graphLine = new astar.Graph(x);
	if(goto_X == 0){
		for(let i = 1; i < mapLength - 1; i++){
			if(i >= 3 && i <= mapLength - 4){
...
exports.next = function(robot_ID, index, socket) {
	...
	if(count > 2){
		...
		if(point[index].x < 3 && point[index].x >= mapLength - 3){
			throwNumberPlate(index, point[index].x, point[index].y);
			reFindRoute(point[index].x, point[index].y, route[index].route_point[route[index].route_point.length - 1].x, route[index].route_point[route[index].route_point.length - 1].y, index, lock)
